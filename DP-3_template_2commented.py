...
class LedThread(QtCore.QThread):
    ...
    led6State = QtCore.pyqtSignal(int)            # define new Signal
...
class MainWindow_EXEC():
    def __init__(self):
        ...
        global led1State_var, led2State_var, led3State_var, led4State_var, led5State_var, led6State_var
        led1State_var = 0
        led2State_var = 0
        led3State_var = 0
        led4State_var = 0
        ...
        self.led_thread.led6State.connect(self.setValue_led6)
    ...
    def setValue_led5(self,state):
        if(ui.comboBox_5.currentText() != "Blue LED"):
            ...
            if(state):
                ...
                ui.label_152.hide()
    def setValue_led6(self,state):
        if(ui.comboBox_2.currentText() != "Red LED"):
            ui.progressBar_17.setValue(0)
            ui.label_149.hide()
        else:
            ui.progressBar_17.setValue(state)
...
class RunThread(QtCore.QThread):
    def __init__(self, parent=None):
        ...
        self.is_running = True
    def run(self):
        time.sleep(0.5) #need this to wait for GUI to load
        def Button(pin):
            try:
                if(str(pin) == "21"):
        ...
        try:
            ...
            ************************************************************
            ...
            ## make sure variables are not being redefined and names make sense
            ## change all the hardcoded stuff to variables -- mainly the time.sleep vals
            global sensor
            sensor = Muscle_Sensor()
            global data_list
            global phase
            global seizure_occurence
            phase = "ns" ## initialize phase to ns - no seizure
            data_list = []
            seizure_occurence = 0 ## initialize num seizures to 0
            time_break = 3 ## the number of seconds the program pauses each loop

            while True: ## infinite loop that constantly detects and tracks seizures
                seizure_detect()
                seizure_counter()
...
n_items = 3 ## rolling average every n data points

incr = True ## emg amplitude increasing
half_cycle = 0 ## half of a amplitude vs time cycle
freq_threshold = 3 ## min num of half cycles per second that indicate tonic seizure activity
seizure_start = 0 ## seizure start time
buzzer = Buzzer(18)

amplitude = 3
seizure_tolerance_A = 2 ## num seizures that pose a moderate risk
seizure_tolerance_B = 3 ## num seizures that pose a high risk
green_LED = LED(12)
yellow_LED = LED(25)
red_LED = LED(24)
min_rest = 1 ## min length of the muscle rest period to be qualified as the clonic phase - (would be 0.18 but adjusted for emulator functionality) 
max_rest = 5 ## max length of the muscle rest period to be qualified as the clonic phase - (would be 0.25 but adjusted for emulator functionality)
avg_freq = 0.1 ## num seconds that indicate how often the rolling average is calculated

## Objective 1
## calculates rolling average using most recent n points from a list of all data points
def roll_avg(data_list):
    if (len(data_list) >= n_items):
        average_list = []
        for i in range(1, (n_items + 1)):
            average_list.append(data_list[-i])
        rolling_average = round(sum(average_list)/(n_items), 2)
...
## Objective 4
## writes the raw data and rolling average to a file
def file(file_name, data_list, data_point, rolling_average):
    file = open(file_name, 'a')
    if len(data_list) == 1: ##only add header first time
        open(file_name, 'w').close() ##clear the contents of the file
        file.write("Raw" + '\t' + "Avg" + '\n\n' + str(round(data_point,2)) + '\t' + str(rolling_average) + '\n')
        file.close()
    else:
        file.write(str(round(data_point,2)) + '\t' + str(rolling_average) + '\n')
        file.close()
        
## Objective 3a
## Detects seizure by measuring the frequency of the emg amplitude over time and comparing it to a min. threshold
## Once seizure is detected a tonic buzzer plays
def seizure_detect():
    global incr
    global half_cycle
    global seizure_start
    global data_list
    global phase
    
    ## ns = no seizure
    if phase == "ns":
        ## keep looping until the frequency threshold is met (i.e until seizure is detected)
        while half_cycle<freq_threshold:
            end_time = time.time() + 1
            while time.time()<end_time: ## for one second count the number of half cycles
                avg1 = None
                while avg1 == None: ## if rolling avg is none then there were not enough data points so theres no valid rolling average so keep looping until theres a valid float
                    time.sleep(avg_freq)

                    ## rolling average 1
                    data_point = sensor.muscle_scaled(10)
                    data_list.append(data_point) 
                    avg1 = roll_avg(data_list)
                    file("EMGdata.txt", data_list, data_point, avg1)
                ...
                data_point = sensor.muscle_scaled(10)
                data_list.append(data_point) 
                avg2 = roll_avg(data_list)  
                file("EMGdata.txt", data_list, data_point, avg2)

                ## count the number of half cycles by counting the instances where the data goes from decreasing to increasing    
                if avg1 < avg2:
                    if incr == False: ## if the data is increasing but was previoulsy decreasing add one to half_cycle
                        incr = True
                        half_cycle += 1
                        
                elif avg1 > avg2:
                    if incr == True: ## update the variable each time it changes from increasing to decreasing
                        incr = False
    
        seizure_start = time.time() ## start time of the seizure

        ## reset variables
        half_cycle = 0
        incr = True
        
        ## start of tonic phase
        phase = "tonic"
        
    ##sound buzzer for tonic
    if phase == "tonic":
        print("Tonic Buzzer On")
        time.sleep(0.2)
        buzzer.on()
        time.sleep(0.5)
        buzzer.off()
...
def seizure_counter():
    ...
    while True:
        ...
        ## calc. rolling average and compare to threshold amplitude to see if the muscle is at rest
        data_point = sensor.muscle_scaled(10)
        data_list.append(data_point) 
        rolling_avg = roll_avg(data_list)
        file("EMGdata.txt", data_list, data_point, rolling_avg)
        
        if rolling_avg <= amplitude:
            rest_start = time.time() ## the start of the muscle rest period
            while True:
                time.sleep(avg_freq)

                ## keep calculating rolling averages until the emg data is higher than the min. threshold for muscle activity
                data_point = sensor.muscle_scaled(10)
                data_list.append(data_point)    
                rolling_avg = roll_avg(data_list) 
                file("EMGdata.txt", data_list, data_point, rolling_avg)
                
                if rolling_avg > amplitude or time.time() - rest_start > max_rest: ## if the seizure is over, it wont break (b/c it will stay a  low amplitude) so have to add the second condition
                    break

            ## if the length of the rest period is within the specified interval then it is in the clonic phase   
            if time.time() - rest_start >= min_rest and time.time() - rest_start <= max_rest: 
                if phase == "tonic": ## only set the clonic start time once (right after the tonic phase)
                    clonic_start = time.time()
                    phase = "clonic"

                 ## sound for clonic phase  
                if phase == "clonic":
                    print("Clonic Buzzer On")
                    buzzer.on()
                    time.sleep(0.1)
                    buzzer.off()

            ## if the rest period is longer than max. seizure rest, then the seiure is over
            elif time.time() - rest_start > max_rest:
                seizure_end = time.time() ## end time of seizure
                seizure_time = seizure_end - seizure_start ## total length of seizure
                seizure_start = 0 ## reset seizure start time back to zero
                phase = "ns" ## reset phase back to no seizure
                seizure_occurence = seizure_occurence + 1  ## add one to the seizure counter
                break 

            ## if the seizure goes on for 5 min (changed it to 30 sec for testing) then emergency services are required
            ## sound for emergency 
            if time.time()-seizure_start>= 30: ## 300
                print("Emergency Buzzer On")
                phase = "!" ## ! to denote emergency phase
                buzzer.on() 
                time.sleep(0.2)
                buzzer.off()
                time.sleep(0.05)
    ...
    ## breaks from the while true after seizure ends
    ## when seizure ends one was added to seizure_occurence
    ## compare the number of seizures to patient specific thresholds and output led to indicate risk level
        ##red: high risk, yellow - mooderate risk, green - no/minor risk
    if (seizure_occurence >= seizure_tolerance_B): 
        green_LED.off()
        yellow_LED.off()
        red_LED.on()
        print("Green LED OFF, Yellow LED OFF, Red LED ON") 
        return 
    
    elif (seizure_occurence >= seizure_tolerance_A):
        green_LED.off()
        yellow_LED.on()
        red_LED.off()
        print("Green LED OFF, Yellow LED ON, Red LED OFF")
        return 
    else:
        green_LED.on()
        yellow_LED.off()
        red_LED.off()
        print("Green LED ON, Yellow LED OFF, Red LED OFF")
